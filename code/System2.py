...
from numpy.random import seed, uniform, exponential, poisson, normal
import numpy as np
import random
import argparse
...
class Packet(Process):
    ...
    def behavior_of_single_packetExp(self, cpu, disk, net):
        ...
        if self.state == "cpu":
            #print("disk")
            #arriveCpu = now()
            Parameters.Arrivals_cpu.append(now())   #add all times of arrival of packet to a list
...
def model():
    ...
    Parameters.steadyState = 0        # change this to 0 when testing out my plotting code for graphing
